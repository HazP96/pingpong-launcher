...
client.on(
  ...
  (channel, tags, message, self) => {
    ...
    if (message === "!ballcount" || message === "!bc") {
      client.say(
        channel,
        `@${tags.username} there is ${ballCount} ball${
          ballCount !== 1 ? "s" : ""
        } remaining`
      );
    }
    ...
    if (
      (userName === "mawrtron" ||
        userName === "tormented_hazard" ||
        userName === "bic_dig_boii" ||
        userName === "kittenclubhouse" ||
        userName === "Tenshi_Artesian" ||
        userName === "Amouranth") &&
    ...
    if (
      ...
      client.say(channel, `This is not a drill!!... JK we are testing`);
      ballsToBeDropped++;
    }
    ...
    if (
      ...
      if (message === "balls out for Harambe") {
        ...
        client.say(
          channel,
          `Thanks @${userName}! Dropping two hairy balls for our fallen brother, Harambe! RIP hairy lad.`
        );
      }
    }
    ...
    if (
      ...
      message.includes("bitties for the kitties!")
    ) {
      // Stream elements zone + custom bit donation message ender for the statement to zone in on.
      // const userNameFromString = message.split(" ")[0]; //OLD kinda broken
      // const bitCount = message.split(" ")[3]; //OLD kinda broken
      const userNameFromString = message.split("with the")[0]; //TODO: make into a constant variable and work out better a way of checking for special character usernames
      const bitCount = message.split("with the")[1].split(" ")[1];
      ...
      if (ballCount == 0) {
        ...
        client.say(
          channel,
          `Thanks @${userNameFromString}! Dropping another ball for the kittens`
        );
